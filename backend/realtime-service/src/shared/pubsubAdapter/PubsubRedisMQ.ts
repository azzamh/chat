import { publishMessage, subscribeToChannel, unsubscribeFromChannel, subscribeToChannelWithId } from '../redisAdapter/pubsub';
...
export class PubsubRedisMQ implements IPubSubMQ {
  ...
  async subscribeToOnlineStatus(
    ...
    subscribeToChannelWithId(this.ONLINE_STATUS_CHANNEL + username, subscriberUsername, async (message) => {
  ...
  async subscribeToTypingStatus(
    ...
    subscribeToChannelWithId(this.TYPING_STATUS_CHANNEL + username, subscriberUsername, async (message) => {
